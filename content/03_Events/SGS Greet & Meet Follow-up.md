---
title: SGS Greet & Meet Follow-up
date: 2025-08-30
tags:
draft: true
---

# Ashok Mathur

## Location
205 Richmond 5th floor
## Role
Vice president, Dean
## Background
Literature
## Quote
> This is your home. A place to land, a place to fly from.

> Take time, make time to contemplate, to really change. Having coffee with friends, sleeping in, dreaming, staring out of window, and having quiet time are all contemplation.

> Precarity, Reconciliation, Innovation, Contemplation, Problem-based learning, Joy, Fragility

> Challenge the system to accomplish what you want.


# Michele Mastroeni

## Location
205 Richmond 5th floor
## Role
Director of Strategic Foresight, Inclusive Design, and Health streams
## Background
Political Science

## About streams
- Strategic Foresight: Human factor, business strategy, system design, design thinking.
- Inclusive Design: Co-design technic, individual problem-solving.
- Health: Health-related technology.


# Jay Irizawa

## Role
Director of Interdisciplinary Media Art
## Recommended books
 - Fear of a Black Universe: An Outsider's Guide to the Future of Physics (Stephon Alexander)
 - A Third University is possible (Yang, K. Wayne)
## Recommended elective
- Thinking through Making

## About advisors
- Selected based on submitted form
- Should be out in March/April 2026
- Seek out tenure/tenure-tracked faculty members, have a talk.



# Simone Jones

## Location
205 Richmond 7th floor, lab @ 6th floor
## Background
40 years ago joined then OCA
## About OCAD and school life
- Celebrating 150th year in 2026
- Other Canadian art schools include: NSCAD (Halifax), AUArt (Alberta), and Emily Carr.
- Most with more than a century's history, but were colleges that didn't provide degrees.
- It was until 25 years ago that art schools became university, so fairly new in terms of university organization.
- Many galleries that you should go to all
- Learn from relationships, start from talking to technicians
- Strong undergraduate cohort, small graduate cohort
- The first year is introduction to cohort, creative technologies, the second year is working towards final thesis
- https://ebookcentral.proquest.com/lib/oculocad-ebooks/reader.action?docID=1766170&c=UERG&ppg=2 (Kate Hartman's book)
## Quotes
> Think of yourself as a creative researcher. Learn how to do both thinking and working, back and forth.



# Other topics

## Student pantry
Ask Jackie Spencer (advocate, legal support)
## Art supply
Ask Jackie Spencer
## Thesis
Ask Elizabeth (Academic Coordinator) about thesis, defense, scheduling.. in the final year
## Funding
Ask Kelcy (Graduate Admissions)

## Event planning
Ask Hannah (Gallery Studio Coordinator).
Have a look at DF specific open shows and try to get involved. Other opportunities include graduate exhibition, critique room (first come first served, can be more frequent). 
Popular timing: summer term and fall second term, which may be booked fast.
## Event announcement
Ask Kim (Graduate Advisor)
## Writing
Ask Indisidu (@5th floor 516) on writing support paper, CVs, grant application, publishing.. 1 on 1 consultation can be booked through booking portal 


# My thoughts



Empowerment is an underlying concept, regardless of the subject, which can be human, or non-human.

Empowerment means to make apparent the intrinsic quality of a material.

Create and change with a specific case.
Don't be satisfied with leaning or summarizing the knowledge.

Create for a specific needs, in the flow, in the real, here and now, together.


My interest in digital futures started out from creative coding and fablab communities (as well as zine).

I am interested in AI open-source, democratization policy making, along with actual coding and visualization. 

In that sense, I may not want to work as a UX designer designing product for others, I want to be a maker-consumer and impart that experience to others.

To be more concrete, in this 2 years, I want to experiment with a range of AI and ALife models. 

I think the word "AI" is in a hype and it now mainly means LLMs. 

But I think it is too powerful in many cases that cause more harm to users than empowering them (not a convivial tool in Illich's word). 

However, AI's definition has shifted and there were many other ways of designing calculation based solutions that benefit users. 

Another important point I want to explore is the long-standing division between AI and ALife. 

In my research framework I want to integrate both components to overcome human-centric understanding of intelligence and actively reference more-than-human intelligence.

~~**I want to put into practice and propose this overarching recipe book to a wider audience.**~~
(Summarization may not suffice as creation, although the learning experience and documentation of doing it from scratch can be useful.
Instead of covering all different methods of model-making, may as well **find a situated case to work out my idea, concretize it**)

What I can think of to integrate the system and policy making component in my final thesis?



